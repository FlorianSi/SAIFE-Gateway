...
![Audience](https://img.shields.io/badge/audience-dpo%2Flegal-blue)
...
This document provides a template for a Data Protection Impact Assessment (DPIA) required under Art. 35 of the GDPR. Read this to understand the systematic processing operations, risk assessments, and mitigation measures for the SAIFE Gateway, and to complete the mandatory DPIA for your school's deployment.
...
*Note: SAIFE Gateway is middleware; the final legal responsibility (Data Controller) lies with the deploying educational institution. Absolute security and complete error-freedom cannot be guaranteed in Artificial Intelligence (AI)-supported systems; the measures described here aim at the best possible risk minimization (best-effort).*
...
*   **Pedagogical Focus Directives (Focus Directives):** Teachers can define specific learning focus instructions for individual students via the dashboard (e.g., "Fractions"). These are transmitted by the school Learning Management System (LMS) as parameters (DslConfig) to the Gateway and temporarily influence the behavior of the AI system.

## 2. Purpose and Legal Basis
The processing serves pedagogical support and the targeted guidance of students in the digital learning environment.
For public schools, Article 6(1)(e) GDPR (performance of a task carried out in the public interest), in conjunction with specific state school laws, typically serves as the legal basis. For the tracking and profiling of minors, explicit consent (Article 6(1)(a), Article 8 GDPR) of the parents/guardians may be mandatory instead, depending on state law.
...
*   **Transient State Management:** The Secure AI For Education (SAIFE) Gateway does not store focus directives or session data persistently on disk. It utilizes a transient, localized `ISessionStore` (e.g., Redis with Time to Live (TTL) constraints) strictly for rate limiting (leaky bucket) and temporary struggle tracking, ensuring the application nodes remain scalable and stateless.
...
*   **Mandatory expiration date (TTL) for directives:** Focus directives receive a mandatory expiration date (`expiresAt`, enforced to a maximum of 30 days by `validateDirective` in `src/SaifeClient.ts`) to reject the unintended accumulation of outdated pedagogical evaluations.
...
*   **Risk 2: Prompt injection via focus directives.** Manipulated or insufficiently verified inputs (e.g., Extensible Markup Language (XML) tags in free-text fields) could bypass AI safety rules and expose the child to unwanted content.
...
*   **Art. 12 (Record-Keeping/Logging):** Structured, batched JavaScript Object Notation (JSON) telemetry logging (without Personally Identifiable Information (PII)) to track system events and policy conflicts.
...
*   **Art. 15 (Robustness & Cybersecurity):** Implemented atomic Leaky Bucket rate limiting against Distributed Denial of Service (DDoS) and continuous context sliding windows against prompt injection (Salami-Slicing).
