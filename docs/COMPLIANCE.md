# Compliance Posture (General Data Protection Regulation (GDPR) & EU AI Act)

![Audience](https://img.shields.io/badge/audience-dpos%2Flegal-blue)
![Status](https://img.shields.io/badge/status-active-orange)
![Updated](https://img.shields.io/badge/updated-2026--07--06-lightgrey)
*Overview of GDPR and EU AI Act compliance for the SAIFE Gateway.*

This document outlines the SAIFE Gateway's compliance posture, written specifically for school Data Protection Officers (DPOs). Read this to understand how data is handled, stored, and exported, as well as the deployer's legal responsibilities under the EU AI Act.
...
|---|---|---|---|
| **Chat History** | Contextual awareness for the Large Language Model (LLM) and Salami-attack inspection. | `ISessionStore` | Transient (e.g., 7-day grace period post-session, configurable). |
...
| **Telemetry** | Anonymous usage metrics for school dashboards. | `ITelemetrySink` | Aggregated counts only; no Personally Identifiable Information (PII) retained. |
| **Roster / PII** | Used in-memory for redaction before sending data to external Application Programming Interfaces (APIs). | Memory only (Transient) | Dropped immediately after request processing. |
...
1. **DPIA / DSFA:** The school must complete a Data Protection Impact Assessment (DPIA) / Datenschutz-Folgenabschätzung (DSFA). See our template: [SAIFE_Data_Protection_Impact_Assessment.md](compliance/SAIFE_Data_Protection_Impact_Assessment.md).
2. **DPA / AVV:** A Data Processing Agreement (DPA) / Auftragsverarbeitungsvertrag (AVV) must be signed with the underlying LLM provider. See our template: [SAIFE_AVV_Template.md](compliance/SAIFE_AVV_Template.md).
3. **Transfer Assessment (TIA):** If the configured LLM endpoint resides outside the EU, the school must conduct a Transfer Impact Assessment (TIA).
